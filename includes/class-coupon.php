...
if ( ! class_exists( 'Affcoups_Coupon' ) ) {
    ...
    class Affcoups_Coupon {
         ...
         */
        function the_image() {
            // since ver 1.1.12 do not display image
        ...
        /**
         * Check whether expiry countdown should be shown or not
         */
        function show_expiry_countdown() {

            if ($this->is_expiry_countdown_enable() )
                return true;

            // Check shortcode atts
            global $affcoups_template_args;

            if ( isset( $affcoups_template_args['show_expiry_countdown'] ) ) {

                if ( 'true' == $affcoups_template_args['show_expiry_countdown'] )
                    return true;

                if ( 'false' == $affcoups_template_args['show_expiry_countdown'] )
                    return false;
            }

            // Check settings
            if ( isset( $this->options['show_expiry_countdown'] ) && '1' == $this->options['show_expiry_countdown'] )
                return true;
        ...
        /**
         * Check whether image should be shown or not , since pro ver 1.1.12
         */
        function hide_image() {
            ...
            if ( isset( $affcoups_template_args['hide_image'] ) ) {

                if ( 'true' == $affcoups_template_args['hide_image'] )
                    return true;

                if ( 'false' == $affcoups_template_args['hide_image'] )
                    return false;
            }
            return false;
        }
         ...
         * @return bool|string
         */
        function is_click_to_reveal_disable() {

            $click_to_reveal = get_post_meta( $this->id, AFFCOUPS_PREFIX . 'click_to_reveal_disable', true );

            return ( ! empty ( $click_to_reveal ) ) ? $click_to_reveal : null;
        }

        /**
         * Check  if social share is disable for a coupon
         *
         * Since ver 1.6.7.6         */
        function is_social_share_disable() {

            $social_share = get_post_meta( $this->id, AFFCOUPS_PREFIX . 'social_share_disable', true );

            return ( ! empty ( $social_share ) ) ? $social_share : null;
        }
        function is_expiry_countdown_enable() {
            $expiry_countdown = get_post_meta( $this->id, AFFCOUPS_PREFIX . 'show_expiry_countdown', true );
            return ( ! empty ( $expiry_countdown ) ) ? $expiry_countdown : null;
        }
        /**
         * Output the code
         */
