	...
	class Affcoups_Settings {
                ...
                'general' => apply_filters( 'affcoups_settings_general',
                    array(
                        ...
                        'fields' => array(
                            ...
                            'vendor_logo' => array(
                                'title' => __( 'Show Vendor Logo', 'affiliate-coupons' ),
                                'callback' => array( &$this, 'vendor_logo_render' )
                            ),
                            'description' => array(
                            ...
                            'social_share' => array(
                                'title' => __( 'Social Share', 'affiliate-coupons' ),
                                'callback' => array( &$this, 'social_share_render' )
                            ),
                            'credits' => array(
		...
		function section_quickstart_render() {
            ...
            <ul style="margin-left: 1em; list-style: disc">
                ...
                <li><a href="#hide_coupon_fields"><?php esc_html_e( 'Hide Coupon Fields', 'affiliate-coupons' ); ?></a></li>
                <li><a href="#display_vendor_list"><?php esc_html_e( 'Display Vendor List', 'affiliate-coupons' ); ?></a></li>
            ...
            <p id="hide_coupon_fields" style="padding-top: 2.4em">
                <strong><?php esc_html_e( 'Hide Coupon Fields', 'affiliate-coupons' ); ?></strong><br/>
                <?php esc_html_e( 'You can hide Coupon fields Description, Title or Image on a shortcode basis as follows:', 'affiliate-coupons' ); ?>
            </p>
            <p>
                <code>[affcoups title_display="hide"]</code>
            </p>
            <p>
                <code>[affcoups description="hide"]</code>
            </p>
            <p>
                <code>[affcoups image_hide="true"]</code>
            
            </p>


            <p id="display_vendor_list" style="padding-top: 2.4em">
                <strong><?php esc_html_e( 'Display Vendor List', 'affiliate-coupons' ); ?></strong><br/>
                <?php esc_html_e( 'You can display Vendor List on frontend on a shortcode basis as follows:', 'affiliate-coupons' ); ?>
            </p>
            <p>
                <code>[affcoups_vendor_list]</code>
            </p>
        ...
        function code_render() {
            ...
            $reveal_code_option = ( isset( $this->options['reveal_code_option'] ) )? $this->options['reveal_code_option'] : '0';
            $login_to_see_code = (isset( $this->options['login_to_see_code'] ) && $this->options['login_to_see_code']=='1')? 1 : 0;
            $login_msg = (isset( $this->options['login_msg'] ))  ? $this->options['login_msg'] : 'Login to see the code!!!';
            $login_url = (isset( $this->options['login_url'] ))  ? $this->options['login_url'] : '';
            ...
            <p>
            <input type="checkbox" id="affcoups_login_to_see_code" name="affcoups_settings[login_to_see_code]" value="1"  <?php echo( $login_to_see_code == 1 ? 'checked' : '' ); ?> />
                <label for="affcoups_login_to_see_code"><?php esc_html_e( 'Ask login to reveal the code', 'affiliate-coupons' ); ?></label>
                <br> <input type="text" id="affcoups_login_msg" name="affcoups_settings[login_msg]" value="<?php echo ($login_msg == "" ? 'Login to see the code!!!' : $login_msg ); ?>">
                <label for="affcoups_login_msg"><?php esc_html_e( 'Message to ask Login', 'affiliate-coupons' ); ?></label>
                <br> <input type="text" id="affcoups_login_url" name="affcoups_settings[login_url]" value=" <?php echo $login_url ?>">
                <label for="affcoups_login_url"><?php esc_html_e( 'URL to Login Page', 'affiliate-coupons' ); ?></label>        
            </p>
                    <p>
                <input type="radio" id="affcoups_reveal_code_option_1" name="affcoups_settings[reveal_code_option]" value="0" <?php echo( $reveal_code_option == 0 ? 'checked' : '' ); ?> />
                <label for="affcoups_reveal_code_option_1"><?php esc_html_e( 'Popup code in current window', 'affiliate-coupons' ); ?></label><br>
                <input type="radio" id="affcoups_reveal_code_option_2" name="affcoups_settings[reveal_code_option]" value="1" <?php echo( $reveal_code_option == 1 ? 'checked' : '' ); ?>>
                <label for="affcoups_reveal_code_code_option_2"><?php esc_html_e( 'Show code in new window with popup, direct current url to offer url', 'affiliate-coupons' ); ?></label> <br>
                <input type="radio" id="affcoups_reveal_code_option_3" name="affcoups_settings[reveal_code_option]" value="2" <?php echo( $reveal_code_option == 2 ? 'checked' : '' ); ?>>
                <label for="affcoups_reveal_code_code_option_3"><?php esc_html_e( 'Show code inside coupon , no popup', 'affiliate-coupons' ); ?></label>
		...
		function dates_render() {
            ...
            $show_expiry_countdown = ( isset( $this->options['show_expiry_countdown'] ) && $this->options['show_expiry_countdown'] == '1' ) ? 1 : 0;
			?>
            <p>
            ...
            <p>
                ...
                <input type="checkbox" id="affcoups_show_expiry_countdown" name="affcoups_settings[show_expiry_countdown]" value="1" <?php echo( $show_expiry_countdown == 1 ? 'checked' : '' ); ?> />
                <?php } else { ?>
                <input type="checkbox" id="affcoups_show_expiry_countdown" name="affcoups_settings[show_expiry_countdown]"  disabled />
                <?php } ?>

                <label for="affcoups_show_expiry_countdown"><?php esc_html_e( 'Show Coupon Expiry Countdown', 'affiliate-coupons' ); ?></label>
            </p>
            <?php affcoups_the_pro_feature_note( __( 'Show Coupon Expiry Countdown', 'affiliate-coupons' ), true ); ?>
            ...
            <p>
                ...
                <label for="affcoups_hide_expired_coupons"><?php esc_html_e( 'Show active coupons only', 'affiliate-coupons' ); ?></label>
            </p>
            <?php
		}
		...
		function title_render() {
            ...
            $title_display_option = ( isset( $this->options['title_display'] ) ) ? $this->options['title_display'] : '';
            ?>
                        <p>
                <select id="affcoups_title_display" name="affcoups_settings[title_display]">
                    <?php foreach ( $title_display_options as $key => $label ) { ?>
                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $title_display_option, $key ); ?>><?php echo esc_attr( $label ); ?></option>
                    <?php } ?>
                </select>
            </p>
     ...
     /**
         * Render vendor_logo settings
         */
		function vendor_logo_render() {

            $vendor_logo = ( isset( $this->options['vendor_logo'] ) && $this->options['vendor_logo'] == '1' ) ? 1 : 0;
            $vendor_logo_display_options = array(
                '' => __( 'Hide', 'affiliate-coupons' ),
                'show' => __( 'Show', 'affiliate-coupons' )
            );

            $vendor_logo_display_option = ( isset( $this->options['vendor_logo'] ) ) ? $this->options['vendor_logo'] : '';
            ?>
                        <p>
                <select id="affcoups_vendor_logo_display" name="affcoups_settings[vendor_logo]">
                    <?php foreach ( $vendor_logo_display_options as $key => $label ) { ?>
                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $vendor_logo_display_option, $key ); ?>><?php echo esc_attr( $label ); ?></option>
                    <?php } ?>
                </select>
            </p>
            <?php
		...
		function description_render() {
            ...
            $description_option = ( isset( $this->options['description'] ) ) ? $this->options['description'] : '';
			?>
            <p>
                <select id="affcoups_description" name="affcoups_settings[description]">
                    <?php foreach ( $description_options as $key => $label ) { ?>
                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $description_option, $key ); ?>><?php echo esc_attr( $label ); ?></option>
                    <?php } ?>
                </select>
            </p>
         ...
         */
        function social_share_render() {
            

            $social_share = ( isset( $this->options['social_share'] ) && $this->options['social_share'] == '1' ) ? 1 : 0;
            
            if ( affcoups_is_pro_version() ){
            ?>
            <p>
                <input type="checkbox" id="affcoups_social_share" name="affcoups_settings[social_share]" value="1" <?php echo( $social_share == 1 ? 'checked' : '' ); ?> />
                <label for="affcoups_social_share"><?php esc_html_e( 'Enable the social share buttons on coupons', 'affiliate-coupons' ); ?></label>
            </p>
            <?php
            }
             affcoups_the_pro_feature_note( __( 'Share coupon', 'affiliate-coupons' ), true ); ?>
            
            <?php

            /* Social Share Msg  Edit option*/
             $social_share_msg = ( ! empty( $this->options['social_share_msg'] ) ) ? esc_attr( trim( $this->options['social_share_msg'] ) ) : __( 'Checkout this offer : ', 'affiliate-coupons-pro' );
             ?>
             <h4><?php esc_html_e( 'Social Share Short Msg : Button Text', 'affiliate-coupons-pro' ); ?></h4>
             <p>
                 <input type="text" class="affcoups-input-click-to-reveal" name="affcoups_settings[social_share_msg]" value="<?php echo esc_attr( $social_share_msg ); ?>"/>
             </p>
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="inside">
                                                ...
                                                <ul>
                                                    ...
                                                    <li><span class="dashicons dashicons-star-filled"></span> <strong><?php _e('Upload coupons via csv file', 'affiliate-coupons'); ?></strong></li>
                                                    <li><span class="dashicons dashicons-star-filled"></span> <strong><?php _e('Option to share coupon on Social Meadia', 'affiliate-coupons'); ?></strong></li>
